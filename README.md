...
[Galactus DB](https://galactusdb.com) · [Driver catalogue](https://galactusdb.com/product/drivers) · [Type mapping](docs/BLUEPRINT.md) · [Spatial guide](docs/SPATIAL.md) · [Issues](https://github.com/galactusdb/galactus-db-cpp-driver/issues)
...
Requirements: **C++17 and CMake 3.16+**, Git, and a reachable Galactus DB instance.
...
Add this to your application’s `CMakeLists.txt`.
...
`Driver(uri, username, password, database = "", timeout_ms = 30000)`. The driver is noncopyable; RAII closes the connection and `close()` is also available.
...
Go uses a deadline for the complete operation. Do not use one driver concurrently. Use separate connections for separate workers.
...
```cpp
auto result = driver.execute_query(
...
metadata. Read a column with `result.records.at(0).at("name").as<std::string>()`. Use unique column aliases because
...
`Value` wraps `std::variant` with native integers, double, bool, UTF-8 strings, `Bytes`, `List`, and `Map`. Read named values with `as<Node>()`, `as<DateTime>()`, or `as<Spatial>()`. `system_clock::time_point` inputs and checked `to_system_time(DateTime)` conversion are supported on the targeted Unix-epoch clocks.
...
auto shape = result.records.at(0).at("shape");
driver.execute_query(
    "CREATE (:Region {name:$name, shape:spatial.fromMap($shape)})",
    {{"name", "Square"}, {"shape", shape}});
std::cout << shape.as<galactus::Spatial>().domain << '\n';
```
...
Server errors expose `DatabaseError.code`, `.metadata`, and `what()`. Distinguish these from protocol, connection,
...
Use [GitHub issues](https://github.com/galactusdb/galactus-db-cpp-driver/issues) for reproducible
